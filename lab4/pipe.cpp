...
// Расчет пропускной способности по формуле: k * sqrt(d^5 / L)
// где k = 1000 - поправочный коэффициент для целых чисел
double Pipe::getCapacity() const {
    if (in_repair || start == 0 || end == 0) {
        return 0.0;  // Труба в ремонте или не соединена
    }

    if (length <= 0) return 0.0;

    // d в метрах для формулы
    double d_m = diameter / 1000.0;  // мм -> метры
    double capacity = 1000.0 * sqrt(pow(d_m, 5) / length);

    return capacity;
}

// Вес для кратчайшего пути (длина трубы, бесконечность если в ремонте)
double Pipe::getWeight() const {
    if (in_repair || start == 0 || end == 0) {
        return std::numeric_limits<double>::max();  // Бесконечность
    }
    return length;
}
...
std::ostream& operator << (std::ostream& out, const Pipe& pipe) {
    ...
    if (pipe.start != 0 && pipe.end != 0) {
        ...
        out << "Пропускная способность: " << pipe.getCapacity() << ";\n";
    }
    out << std::endl;
    return out;
}