...
void Network::CalculateMaxFlow() {
    if (stations.size() < 2) {
        std::cout << "Необходимо минимум 2 станции для расчета потока" << std::endl;
        return;
    }

    std::cout << "\n=== Расчет максимального потока (Форд-Фалкерсон) ===" << std::endl;

    // Выбор исходной и стоковой станций
    std::cout << "\nСписок станций:" << std::endl;
    for (const auto& station_pair : stations) {
        std::cout << "ID: " << station_pair.first << " - "
            << station_pair.second.getName() << std::endl;
    ...
    std::cout << "\nВыберите ID станции-источника: ";
    int source = check_station_id(stations);

    std::cout << "Выберите ID станции-стока: ";
    int sink = check_station_id(stations, source);

    if (source == sink) {
        std::cout << "Источник и сток не могут быть одной станцией!" << std::endl;
        return;
    }

    // Создаем граф для расчета потока
    std::unordered_map<int, std::unordered_map<int, double>> capacity;

    // Заполняем граф пропускными способностями
    for (const auto& pipe_pair : pipes) {
        ...
        if (pipe.getStart() != 0 && pipe.getEnd() != 0) {
            ...
            if (cap > 0) {
                capacity[u][v] += cap;  // Суммируем capacity если несколько труб
            }
        }
    }

    // Проверяем, есть ли путь от источника к стоку
    bool hasPath = false;
    for (const auto& neighbor : capacity[source]) {
    ...
    if (!hasPath && capacity[source].empty()) {
        std::cout << "Нет соединений от источника к стоку!" << std::endl;
        return;
    }
    ...
    while (true) {
        ...
        if (!bfsForMaxFlow(source, sink, parent)) {
        ...
        for (int v = sink; v != source; v = parent[v]) {
            ...
            // Ищем capacity между u и v
            double residual = 0;
            if (capacity.find(u) != capacity.end() && capacity[u].find(v) != capacity[u].end()) {
    ...
    std::cout << "Максимальный поток от станции " << source
        << " к станции " << sink << ": " << maxFlow << std::endl;

    // Выводим потоки по трубам
    std::cout << "\nПотоки по трубам:" << std::endl;
    bool hasFlows = false;
    for (const auto& pipe_pair : pipes) {
        const Pipe& pipe = pipe_pair.second;
        int u = pipe.getStart();
        ...
        if (u != 0 && v != 0 && flow.count(u) && flow[u].count(v) && flow[u][v] > 0) {
            std::cout << "Труба ID " << pipe_pair.first
                << " (" << u << " -> " << v << "): "
                << flow[u][v] << " / " << pipe.getCapacity()
                << " (" << (pipe.getInRepair() ? "в ремонте" : "рабочая") << ")" << std::endl;
            hasFlows = true;
        }
    }

    if (!hasFlows) {
        std::cout << "Нет потоков по трубам (возможно, все трубы в ремонте)" << std::endl;
    }
}

bool Network::bfsForMaxFlow(int source, int sink, std::unordered_map<int, int>& parent) {
    std::set<int> visited;
    std::queue<int> q;
    ...
    // Создаем граф текущих остаточных пропускных способностей
    ...
    while (!q.empty()) {
        ...
        if (residual.find(u) == residual.end()) continue;

        for (const auto& neighbor_pair : residual[u]) {
            int v = neighbor_pair.first;
            double capacity = neighbor_pair.second;
    ...
    return false;  // Не нашли путь до стока
}

void Network::CalculateShortestPath() {
    if (stations.size() < 2) {
        std::cout << "Необходимо минимум 2 станции для расчета пути" << std::endl;
        return;
    }

    std::cout << "\n=== Расчет кратчайшего пути (Дейкстра) ===" << std::endl;

    // Выбор начальной и конечной станций
    std::cout << "\nСписок станций:" << std::endl;
    for (const auto& station_pair : stations) {
        std::cout << "ID: " << station_pair.first << " - "
            << station_pair.second.getName() << std::endl;
    ...
    std::cout << "\nВыберите ID начальной станции: ";
    int start = check_station_id(stations);

    std::cout << "Выберите ID конечной станции: ";
    int end = check_station_id(stations, start);

    // Запускаем алгоритм Дейкстры
    auto path = dijkstra(start, end);

    if (path.empty() || path.back().first != end) {
        std::cout << "\nПуть от станции " << start << " к станции " << end
            << " не найден!" << std::endl;
        std::cout << "Возможные причины:" << std::endl;
        std::cout << "1. Нет соединения между станциями" << std::endl;
        std::cout << "2. Все пути содержат трубы в ремонте" << std::endl;
        std::cout << "3. Станции не соединены напрямую или через другие станции" << std::endl;
        return;
    }

    // Выводим результат
    std::cout << "\n=== Кратчайший путь ===" << std::endl;
    std::cout << "От станции " << start << " к станции " << end << ":" << std::endl;

    double totalDistance = 0;
    std::cout << "\nМаршрут:" << std::endl;

    for (size_t i = 0; i < path.size(); i++) {
        std::cout << "Станция " << path[i].first << " ("
            << stations.at(path[i].first).getName() << ")";

        if (i < path.size() - 1) {
            // Находим трубу между текущей и следующей станцией
            int from = path[i].first;
            int to = path[i + 1].first;
            bool foundPipe = false;

            for (const auto& pipe_pair : pipes) {
                const Pipe& pipe = pipe_pair.second;
                if ((pipe.getStart() == from && pipe.getEnd() == to) ||
                    (pipe.getStart() == to && pipe.getEnd() == from)) {
                    std::cout << " --[Труба ID: " << pipe_pair.first
                        << ", Длина: " << pipe.getLength() << " км"
                        << ", Диаметр: " << pipe.getDiameter() << " мм"
                        << ", " << (pipe.getInRepair() ? "в ремонте" : "рабочая")
                        << "]--> ";
            ...
            if (!foundPipe) {
                std::cout << " --[Соединение без трубы]--> ";
            }
        }
    }

    std::cout << "\n\nИтоговая статистика:" << std::endl;
    std::cout << "Общая длина пути: " << totalDistance << " км" << std::endl;
    std::cout << "Вес пути (расстояние): " << path.back().second << " км" << std::endl;
    std::cout << "Количество перегонов: " << path.size() - 1 << std::endl;
}

std::vector<std::pair<int, double>> Network::dijkstra(int start, int end) {
    ...
    for (const auto& station_pair : stations) {
        int id = station_pair.first;
        distance[id] = std::numeric_limits<double>::max();
        previous[id] = -1;
        unvisited.insert(id);
    }
    ...
    while (!unvisited.empty()) {
        ...
        if (current == -1 || current == end) {
            break;  // Достигли конечной точки или нет пути
        }

        unvisited.erase(current);

        // Обновляем расстояния до соседей (неориентированный граф)
        for (const auto& pipe_pair : pipes) {
            const Pipe& pipe = pipe_pair.second;

            // Проверяем исходящие ребра из current
            if (pipe.getStart() == current && unvisited.count(pipe.getEnd()) > 0) {
                double weight = pipe.getWeight();
                if (weight < std::numeric_limits<double>::max()) {
                    double alt = distance[current] + weight;

                    if (alt < distance[pipe.getEnd()]) {
                        distance[pipe.getEnd()] = alt;
                        previous[pipe.getEnd()] = current;
                    }
                }
            }

            // Проверяем входящие ребра в current (для неориентированного графа)
            if (pipe.getEnd() == current && unvisited.count(pipe.getStart()) > 0) {
                double weight = pipe.getWeight();
                if (weight < std::numeric_limits<double>::max()) {
                    double alt = distance[current] + weight;

                    if (alt < distance[pipe.getStart()]) {
                        distance[pipe.getStart()] = alt;
                        previous[pipe.getStart()] = current;
                    }
                }
            }
        }
